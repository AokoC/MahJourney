...
>
> [このファイル](../batch-upload/batch.py) を参照してください。このプログラムは、改行で区切られた大量の.txt形式の問題を、本ソフトウェアのdata.json形式に変換するのに役立ちます。ただし、.txtファイルは依然として自分で入力する必要があります（そしてインポートには.zipが必要なことを忘れていたので、その後手動で圧縮する必要があるかもしれません）。
