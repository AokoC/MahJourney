...
可以即时看到设置好的答案（对错由红绿色的框线区分），并且左侧栏的【题干】会变成【讲解】。
...
很适合那些有着复杂场况、需要做副露判断，甚至是算点和见逃相关的何切。不过图片本身是不支持点击的。
...
管理页面。在此处可以：
- 以列表或网格形式浏览何切题库
- 选中某道何切，修改，或观看其生涯数据（如果设置里有开启）
- 批量选中多道何切，进行删除、导出等操作
  - 会连同对应的数据和图片一起打包成 .zip，保存至 saves/export 文件夹
- 导入何切集
  - 只接受导出产生的 .zip
...
[Release](https://github.com/AokoC/MahJourney/releases)

## 待实现

从可能到不可能排列：

- [ ] 生涯页面，可以查看一些特殊的数据，例如总作答数，总时长之类
- [ ] 网页版本
- [ ] 枚数、进张数等分析，无需再手动填写相关讲解
- [ ] 手机应用版本
- [ ] 图像识别，无需再手动选择手牌

这个项目本身是为了满足我自身需求而做的，因此大概率不会再实装以上这些项目了，仅仅是列出曾想过的功能……如果你有兴趣的话，欢迎 PR。

## 为什么你有个何切三百？

这是我自己敲的。你可以同样在 [Release](https://github.com/AokoC/MahJourney/releases) 找到它，是一个 .zip 文件；打开本软件后，到管理界面导入该文件即可。就当是一个可选的内置题库。
